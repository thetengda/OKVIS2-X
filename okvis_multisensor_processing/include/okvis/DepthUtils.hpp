// The following code is mostly taken from: https://github.com/opencv/opencv_contrib/blob/4.x/modules/rgbd/src/depth_registration.cpp

#ifndef INCLUDE_OKVIS_DEPTHUTILS_HPP
#define INCLUDE_OKVIS_DEPTHUTILS_HPP
...
namespace okvis {
  ...
  template<typename DepthDepth>
  void
  performRegistration(const cv::Mat_<DepthDepth> &unregisteredDepth,
                      const cv::Matx33f &unregisteredCameraMatrix,
                      const cv::Matx33f &registeredCameraMatrix,
                      const cv::Mat_<float> &registeredDistCoeffs,
                      const cv::Matx44f &rbtRgb2Depth,
                      ...
                      cv::Mat &registeredDepth)
  {
    ...
    registeredDepth = cv::Mat_<DepthDepth>(outputImagePlaneSize, noDepthSentinelValue<DepthDepth>());

    // Figure out whether we'll have to apply a distortion
    bool hasDistortion = (cv::countNonZero(registeredDistCoeffs) > 0);
    ...
    {
      cv::Mat_<cv::Point3f> point_tmp(outputImagePlaneSize,cv::Point3f(0.,0.,0.));
      ...
      cv::perspectiveTransform(point_tmp, transformedCloud, initialProjection);
    }
    ...
    const cv::Rect registeredDepthBounds(cv::Point(), outputImagePlaneSize);
    ...
    for( int y = 0; y < transformedCloud.rows; y++ )
    {
      if (hasDistortion)
      {
        ...
        cv::projectPoints(transformedCloud.row(y),
                          cv::Vec3f(0,0,0),
                          cv::Vec3f(0,0,0),
  ...
  void registerDepth(cv::InputArray unregisteredCameraMatrix, cv::InputArray registeredCameraMatrix, cv::InputArray registeredDistCoeffs,
                   cv::InputArray Rt, cv::InputArray unregisteredDepth, const cv::Size& outputImagePlaneSize,
                   cv::OutputArray registeredDepth, bool depthDilation)
  {
    ...
    cv::Matx33f _unregisteredCameraMatrix = unregisteredCameraMatrix.getMat();
    cv::Matx33f _registeredCameraMatrix = registeredCameraMatrix.getMat();
    cv::Mat_<float> _registeredDistCoeffs = registeredDistCoeffs.getMat();
    cv::Matx44f _rbtRgb2Depth = Rt.getMat();


    cv::Mat &registeredDepthMat = registeredDepth.getMatRef();
    ...
    switch (unregisteredDepth.depth())
    {
      ...
      default:
      {
        CV_Error(cv::Error::StsUnsupportedFormat, "Input depth must be unsigned short, float, or double.");
      }
    }
...
#endif // INCLUDE_OKVIS_DEPTHUTILS_HPP