...
Realsense::Realsense(SensorType sensorType, bool enableRgb, float rgbExposure)
    : streaming_(false), started_(false), sensorType_(sensorType), enableRgb_(enableRgb), rgbExposure_(rgbExposure) {}
...
: streaming_(false), started_(false), sensorType_(sensorType),
  ...
  irSize_(irSize), rgbSize_(rgbSize), rgbExposure_(rgbExposure) {}
...
bool Realsense::startStreaming() {
  ...
  // set RGB exposure
  if (enableRgb_ && (rgbExposure_ > 0.f)) {
    auto rgb_sensor = selected_device.first<rs2::color_sensor>();
